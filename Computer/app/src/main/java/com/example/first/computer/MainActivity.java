...
public class MainActivity extends Activity implements OnClickListener {
    ...
    // 存用户输入的数字
    private List<Double> value_list = new ArrayList<Double>();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.bt_div:
                ...
                break;
            case R.id.bt_result:
                // 防止用户没输入数字，或者只输入了一个数，就按=
                if (value_list.size() > 0 && operator_list.size() > 0 && can_operate_flag)
                {
    ...
    private void num_down(String num) {
        ...
        if ((num.equals(".") && editText1.getText().toString().indexOf(".") < 0) || !num.equals("."))
        {
            // 如果用户输入的是小数点.，则要判断当前已输入的数字中是否含有小数点.才允许输入
            if((editText1.getText().toString().equals(""))&&num.equals("."))
            {
                editText1.setText("0");
            }
